const javascriptValidators = {
    removeWhitespace: (code) => code.replace(/\s+/g, ''),
    
    validateLevel1: (code) => {
        const normalizedCode = javascriptValidators.removeWhitespace(code);
    ...
    validateLevel3: (code) => {
        const normalizedCode = javascriptValidators.removeWhitespace(code);
        const expectedCode = "dequeue(){if(this.isEmpty())returnnull;constremovedNode=this.first;this.first=this.first.next;if(this.first===null){this.last=null;}this.size--;returnremovedNode.value;}";
    ...
    validateLevel4: (code) => {
        const normalizedCode = javascriptValidators.removeWhitespace(code);
        const expectedCode = "isEmpty(){returnthis.size===0;}";
    ...
    validateLevel5: (code) => {
        const normalizedCode = javascriptValidators.removeWhitespace(code);
        const expectedCode = "constfila=newQueue();fila.enqueue(10);fila.enqueue(20);fila.enqueue(30);";
        ...
        // Lista de possíveis erros
        const errors = [];
        if (!normalizedCode.includes("constfila=newQueue();")) {
            errors.push("A criação da instância 'fila' da classe 'Queue' está ausente ou incorreta.");
        }
        if (!normalizedCode.includes("fila.enqueue(10);")) {
            errors.push("A primeira chamada de 'enqueue(10)' está ausente ou incorreta.");
        }
        if (!normalizedCode.includes("fila.enqueue(20);")) {
            errors.push("A segunda chamada de 'enqueue(20)' está ausente ou incorreta.");
        }
        if (!normalizedCode.includes("fila.enqueue(30);")) {
    ...
    validateLevel6: (code) => {
        const normalizedCode = javascriptValidators.removeWhitespace(code);
        const expectedCode = "fila.dequeue();fila.dequeue();";
        ...
        // Lista de possíveis erros
        const errors = [];
        if (!normalizedCode.includes("fila.dequeue();")) {
            const dequeueOccurrences = normalizedCode.split("fila.dequeue();").length - 1;
...
export default javascriptValidators;